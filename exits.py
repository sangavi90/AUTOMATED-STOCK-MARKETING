...
class exit_on_trigger(APIView):
        def post(self,request):
            ...
            for leg in Leg.objects.filter(orderid=orderid):
                ...
                conn.request("POST", "/rest/secure/angelbroking/order/v1/getLtpData", json.dumps(payloads), headers) #To get ltp data of contract based NSE
                ...
                if float(leg.triggerprice)== triggerltp:
                    # Update order status to 'placed'
                    
                    smartApi.placeOrder(exit_trigger_params)
                    ...
                    return Response("LTP checked and incomplete trigger")
...
class ExitOnTrigger(APIView):
    ...
    def put(self, request):
        ...
        try:
            leg = Leg.objects.get(orderid=orderid)
            leg.stop_trigger_flag = True
            leg.save()
            return Response("Trigger started successfully")
...
################################################
class exit_on_stoploss(APIView):
        def post(self,request):
            ...
            for leg in Leg.objects.filter(orderid=orderid):
                ...
                if float(leg.triggerprice)== triggerltp:
                    ...
                    smartApi.placeOrder(exit_stoploss_params)
